...
(if (not (rp-var-object 'power_law_parameters_solving_method_rp))(rp-var-define 'power_law_parameters_solving_method_rp #f 'boolean #f))
;;
(define  gui-distribution
(let ((panel #f)(minGSD)(maxGSD)(powerLawParametersSolvingMethod))
(define (update-cb . args)    
(cx-set-real-entry minGSD (%rpgetvar 'min_gsd_rp))      
(cx-set-real-entry maxGSD (%rpgetvar 'max_gsd_rp))
(cx-set-toggle-button powerLawParametersSolvingMethod (%rpgetvar 'power_law_parameters_solving_method_rp))  
  )    
(define (apply-cb . args)      
(rpsetvar 'min_gsd_rp (cx-show-real-entry minGSD)) 
(rpsetvar 'max_gsd_rp (cx-show-real-entry maxGSD)) 
(rpsetvar 'power_law_parameters_solving_method_rp (cx-show-toggle-button powerLawParametersSolvingMethod)) 
(%udf-on-demand "tutmam_transfer_settings_manually_aerosol_distribution::tutmamudf")
   )    
(lambda args  
(if (not panel) 
(let ((table)(table2) (form)(radioButton1)(radioButtonBox))
(set! panel (cx-create-panel "Particle distributions" apply-cb update-cb))    
(set! table (cx-create-table panel "Log-normal" 'border #t 'below 0 'right-of 0))
(set! form (cx-create-frame table "" 'border #f))
...
(set! table2 (cx-create-table panel "Power law" 'border #t 'below 0 'right-of table))
(set! radioButtonBox (cx-create-button-box table2 "Parameter solving method" 'radio-mode #t ))
(set! radioButton1 (cx-create-toggle-button radioButtonBox "Levenberg-Marquardt iteration algorithm"))
(set! powerLawParametersSolvingMethod (cx-create-toggle-button radioButtonBox "Interpolation table"))
(cx-set-toggle-button radioButton1 #t)  
(cx-set-toggle-button powerLawParametersSolvingMethod (%rpgetvar 'power_law_parameters_solving_method_rp))  
(cx-create-button panel "Apply" 'panel-button #t 'activate-callback apply-cb)
)  
) (cx-show-panel panel)    )))
...
(cx-add-item menu "Particle distributions" #\U #f cx-client? gui-distribution)
